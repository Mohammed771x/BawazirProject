...
| Date | Work |
|------|------|
| 2026-09-15 | **Applying one migration to production took four silent failures** (ADR-079), each of which the next migration will meet again. The documented command named `ConnectionStrings__WordOs`, but the design-time factory reads `WordOsMigrations` first — already in user-secrets on any machine that has run the backend locally — so `dotnet ef` migrated the laptop and printed *"the database is already up to date."* True, of the wrong database. Then Neon's default `neondb` is not the app's `wordos`: migrating it replayed all 23 migrations from `InitialSchema` and succeeded, against 8 MB of nothing, while the real database sat next to it with 46 users and 234,359 lexicon rows. Then the `-pooler` host refused the startup parameter needed to assume a role. Then `neondb_owner` turned out not to own the tables — `wordos_migrator` does, and a foreign key to `users` needs `REFERENCES` from its owner; the way through is that `neondb_owner` holds `admin_option` and can borrow the role for one command, handing it back with `GRANTED BY` so the revoke does not remove the *original* grant. And the one that fails at runtime rather than at migration time: `ALTER DEFAULT PRIVILEGES` was attached to `neondb_owner` while tables are created by `wordos_migrator`, so **every new table reaches `wordos_app` with no privileges** and 500s the first time a learner touches it. `WordOsDbContextFactory` now prints `host/database/role` before touching anything — one line, which caught three of the four on sight. A probe that appeared to confirm the fix had not: `/password/forgot` with an unregistered address returns `202` without ever reaching the new table, so verification now uses a registered email and checks for the row. Password reset is live on production, the code row stored Argon2id-hashed, and the role membership restored to exactly as found. |
| 2026-09-15 | **Forgotten-password recovery** (ADR-078) — a six-digit code by email, which the project had no way to send: no SMTP, no provider, no key. Brevo, chosen because it verifies a single *sender address* rather than a whole domain, behind an `IEmailSender` that makes the provider a class rather than a refactor. A code rather than a link: deep links need platform plumbing and break when mail is read on a different device from the app. Six digits is a million possibilities, so three limits bound it and each closes a hole the others leave — 15-minute expiry, five attempts per code, and one live code at a time (otherwise three taps mean fifteen guesses). Stored Argon2id-hashed, not SHA-256, because a million SHA-256s is an eye-blink; affordable only because nothing looks a code up *by its hash* — redemption finds the row by user and verifies one candidate. The property that shaped the rest: neither endpoint may say whether an address is registered, **including when the provider is down** — only a registered address causes a send, so an error that surfaced would be the oracle. Every reset failure is one answer, and an unknown email still pays for a verification so the timing does not differ. The reset returns no tokens and revokes every refresh token. Backend: new `password_reset_codes` table + migration, 12 integration tests. Mobile: two-step screen on one route (the second step needs the first step's email), the router guard exempted so a locked-out learner can actually reach it, mock-backend simulation of all of it, 6 widget tests. Full suites green — 400 API, 94 domain, 410 Flutter — except the pre-existing `DailyReminderTests` horizon failure, still untouched. |
| 2026-09-14 | **The health check was the hosting bill** (ADR-077). Neon reported 80 of the month's 100 compute-hours used by the 14th, and none of it was learner traffic: a serverless database bills the time it is *awake*, suspends after 5 minutes idle, and `/health/ready` — which opens a real connection, on purpose — was being probed every 5 minutes by cron-job.org *and* by Render's own health check, because `docs/09-DEPLOYMENT.md` recommended exactly that in §4 and configured it in §3. `0.25 CU × 324 hours = 81`, which is the number Neon showed. At that rate the allowance dies on the 17th of every month and the app stops. Fixed in both halves: the deployment guide now points the keep-alive at `/health/live` (no connection, 10-minute interval) with a separate hourly monitor on `/health/ready`, and the service itself now throttles the real check to once per `Capacity__ReadinessDatabaseCheckSeconds` (one hour by default, `0` to ask every time — rule R3). Only *successes* are cached, so a database coming back is seen at once, and the response carries `databaseCheckedSecondsAgo` so a cached `ok` is never mistaken for a fresh one. Documentation alone would not have held: the guide already had `/health/live` and argued against it. Five unit tests pin the properties that matter — a burst costs one round trip, the window really does expire, failures are not remembered. Unrelated and pre-existing: `DailyReminderTests.Both_times_of_day_are_sent_for_every_day_of_the_horizon` fails on a clean tree (expected 7 days of reminders, got 6), untouched here. |
| 2026-09-13 | Two changes the product owner asked for. **The dictionary stopped being a gate** (ADR-075): a learner may now add a word the lexicon has never heard of, writing the meaning themselves. The lexicon is a machine join of WordNet and two CEFR lists and it has holes — `deepfake`, `microservice`, `doomscroll` — and the word somebody most wants to add is disproportionately the one that fell down one. The three facts a word needs to enter the pipeline (level, part of speech, English definition) now come from the meaning checker when the lexicon cannot supply them, and the checker answers a second question first: *is this English at all*. Unlike a contested meaning, that refusal cannot be overridden — nothing downstream can teach `asdfghjkl` — so instead it hands back the spelling it thinks was meant, one tap from being tried. A word the lexicon *does* hold is never re-judged, pinned by a test where the stub checker denies everything and the dictionary entry still goes in. R2 holds throughout: the part of speech is filtered through a fixed list and the band through the CEFR ladder, so `gerundive`/`Z9` reaches nothing. Verified against real Gemini: `deepfake` = `تزييف عميق` saved as noun/C1 with a real definition, `asdfghjkl` refused as *"مجرد سلسلة من الحروف المتجاورة على لوحة المفاتيح"*, `recieve` refused with `correctedWord: receive`, `microservice` = `طاولة` refused on meaning. **And daily reminders** (ADR-076), local notifications with no Firebase and no push: two a day, morning and evening. The hard part is that the phone fires them offline with the app closed, so nothing can be counted at the time — the server therefore computes a week of them in advance, one per slot per day, each carrying the count that will be true *at that moment*, and the phone only schedules and translates. A word waiting out a spaced gap is `NOTHING_DUE` on Tuesday and `WORDS_DUE` on Thursday in the same response. Three message kinds, because "you have 0 words ready" is the wrong thing to say both to somebody who never started and to somebody who finished yesterday. Wall-clock times so "morning" means the learner's morning; past slots dropped at both ends; every refresh replaces rather than appends; cancelled on sign-out; a switch in Settings. 404 Flutter tests, 384 backend API + 94 domain, 103 AI-service. |
| 2026-08-23 | A conversation now always ends on a goodbye (ADR-070). It ends for two reasons — every word used, or the turn limit — and only the first had a closing turn; the second just stopped, with the tutor's last line left as "try to use the word …" and the result screen over the top of it. The learner who hits that path is the one who never managed a word, so the conversation they got wrong was also the one that ended mid-sentence. Both endings now ask for the closing turn, and it is told which words were missed precisely so it does not congratulate them on those — while being forbidden to list them or apologise, because the evaluation explains that afterwards in Arabic. Also audited every prompt for the interests rule: they appear in exactly two, both now open with *natural use comes first* and rank interests last, with a test that neither can lose it. And session persistence, reported as broken, was pinned instead: a test signs in, discards the entire widget tree, boots the app again against the same keystore and asserts the hub — no sign-in form. 369 Flutter tests, 443 backend, 96 AI-service. |
| 2026-08-23 | Speaking (ADR-069). The tutor was being handed the remaining words as bare strings, so shown `can` it could not tell the modal from a tin and asked a learner interested in technology what a computer can do — a question the word cannot answer. The English definition and part of speech now travel with each word and the prompt is told to read the sense before writing the question; interests moved to the foot of the prompt as lowest priority. Verified live: the same setup now produces "Do you ever buy a cold drink or food in a can when you are at the grocery store?". Then dictation: a pause restarted the transcript and a long answer overwrote itself, both because every recogniser closes its session on silence or after a minute and reopens it empty, while this service kept only the latest result. Turns are now assembled from segments — a final result is appended and the session reopened — with the learner's intent held separately from the platform's session, `cancelOnError` off so a transient error costs a few words rather than the turn, and reopening bounded at forty silent restarts. And closing the microphone now opens a review: the transcript in a text field with *Record again* and *Send*, so a misheard word can be fixed before anything is sent or any AI call spent. Five dictation tests and two review tests. 368 Flutter tests, 442 backend, 94 AI-service. |
| 2026-08-23 | Passages are now *built* rather than merely written, and Listening caught up with Reading (ADR-068). From B1 up the prompt asks for the shape an exam passage actually has — an opening that states the subject, body paragraphs each led by their own topic sentence, a close that lands it, and visible joins; below B1 a plainer rule, because eighty words with a thesis and a conclusion is a parody rather than a beginner's text. Paragraph starts travel as indexes beside the sentences, never as blank elements inside them, which would have shifted every target-word index. Measured: A1 one paragraph, B1 four, C1 and C2 six, coverage and timing unchanged. Listening then gained the title, choose → تحقق → next, and the recording reachable from the questions — refused earlier on the grounds that replaying hands over the answers, allowed now because a question you cannot re-listen to is a memory test. And a scrubber: TTS has no playhead, so the clip is spoken sentence by sentence and those boundaries are the seek points — drag anywhere, jump to either end, and switching to the slow voice re-speaks the current line instead of restarting. 361 Flutter tests, 442 backend, 94 AI-service. |
| 2026-08-23 | Reading went slow the moment its passages were sized properly, and changing level began failing with "the passage could not be rewritten" (ADR-067). Two costs, both from the extra length: one call was being asked for a 430-entry glossary — 43 seconds and 18,000 tokens for it — and the repair batches were running one after another for no reason, since each asks about a different set of words. Worst case they composed to 77 seconds and crossed the backend's AI budget. Passages over 260 words are now written without a glossary at all (the schema drops the field, so the model spends everything on the prose) and the batches are dispatched together on a thread pool. Re-telling B1 → C2 went from 43–77s to **22s**, a fresh C2 from 45s to **21s**, and generation time is now roughly flat across the whole ladder — the part that grows with length is the part that runs in parallel. Timeouts came back down with it: AI budget 80s → 60s, client 120s → 90s. Zero unanswerable taps throughout. |
| 2026-08-23 | Reading passages resized, re-aimed and titled (ADR-066). Length was set in sentences from a table checked against nothing — thirteen at C2 — so it is now measured against what public examinations actually use (A2 Key caps a text at 230 words; B2 First's Part 5 runs 500–600; C1 Advanced's paper is 3,000–3,500) and asked for as a **sentence count**, because a model asked for 720 words returns 361 and asked for 29 sentences returns 29. The prompt no longer opens with the learner's interests: the target words and a natural-use rule come first, interests are marked lowest priority, and the model is told a passage on the wrong subject is fine while one that misuses a word is not — verified on the reported case (`can` the tin, interest `technology`), which now yields a lunch scene rather than a forced paragraph about programming. The generator also writes a title, carried through a new `ContentTitle` column to a heading above the passage. Three ceilings moved with the length, each measured first: the glossary column (a C2 passage needs 30,000 characters of it), the repair pass (now batched — one capped call left 227 unanswerable taps at C2), and the AI budget 25s → 80s with the client's 90s → 120s, since a C1 passage measures 35 seconds to write. Sweep over all eleven bands, two topics each: 84 words at A1 to 765 at C2, zero unanswerable taps anywhere. 442 backend tests, 94 AI-service, 354 Flutter. |
| 2026-08-23 | Tapping a word in a Reading passage was reported as giving the right meaning at some levels and a list of dictionary senses at others (ADR-065). The plumbing was sound end to end — prompt, AI service, backend, storage and client all carry the glossary, and the re-tell path carries it too. What was missing was any guarantee: nothing between the model and the learner ever counted the glossary against the passage, so a flash-lite answer that glossed the interesting words and skipped `was`/`before`/`quiet` shipped exactly like a complete one, and every skipped word became a six-sense lexicon lookup. The service now tokenises its own passage with the client's word rule and asks once more for the words it left out, by name and in their sentences; a failed repair returns what it has rather than dropping the learner into the fallback, which has no glossary at all. Five new tests, and `tools/glossary_coverage.py` measures every level and exits non-zero on any unanswerable tap. A second defect surfaced in the same area: passage length knew only the six whole bands, so all five `_PLUS` levels fell through to the default — an A1+ learner was reading a B1-length text. Then the same section was tested on a device and reported broken three ways — a four-sentence "B1" passage, re-levelling refusing to work, and `all` answering with every meaning it has. All three were one fact: the AI service was down (no key locally), so every generation took `FallbackContent` — which ignored the level and carried no glossary at all. The fallback now carries a hand-written glossary covering every word of its own fixed sentences, scales its length with the level, and no longer splices the lexicon's English definition into the passage where nothing could gloss it. Three backend tests, one of which walks all eleven bands over real HTTP and asserts the passage's words and the glossary's words are the same set; the AI test double now glosses whatever it wrote, because a stub that glossed three words let the suite pass on a passage most of whose taps failed. Verified against the real model once a working key was in place: 33 passages across all eleven bands, three topics each, **zero unanswerable taps** — with the repair pass firing on 17 of 34 generations, short by one to four words each time and closing every one. Half of all passages would otherwise have shipped a hole, which is exactly what "works at C2, not at A1" looks like from a device. Then walked on the simulator against the live local stack: a practice passage generated for real (the repair pass firing on that very request — 98 entries, 0 unglossed), `Modern` answering **حديث [صفة]** and `automate` answering **أتمتة [فعل]**, the level changed B1 → C2 and the passage re-told with its glossary intact, then choose → تحقق → التالي and the book icon back to the passage and back to the same question with the choice still selected. 442 backend tests, 88 AI-service tests. The eleven-level sweep against real Gemini is still to run — the local `.env` has no key. |
| 2026-08-23 | Two Reading changes from the device, both client-side (ADR-064). Answering is now choose → *Check* → *Next*: a tap only selects, so a mis-tap no longer spends the attempt, and the choice survives a failed submission — the retry is pressing Check again. And the passage is reachable from the questions: a book icon in the app bar reopens the text and *Back to the questions* returns to the same question, with the level still locked because the answers belong to that passage. Deliberately Reading only — Listening shares the screen and replaying its clip mid-question would hand over the answers. No backend or contract change. Three existing walkthroughs had encoded the one-tap flow and were updated; two new tests pin the new one, including that no verdict appears before Check. 354 Flutter tests. |
...
| 2026-08-21 | Exception handling, end to end, from the learner's side. Audit first: the API can answer with **46 error codes and the app localized 17**, so 29 of them reached an Arabic learner as an English sentence — which ADR-035 already says reads as a crash. Worse, several screens bypassed the lookup entirely and printed the server's text verbatim, and the session screen printed `e.toString()` — a Dart error message, in English, occasionally quoting the data that broke it. All 46 are now said in both languages, plus the twelve transport codes the client raises for itself, and **every** display path goes through one table. Three structural fixes behind the wording. **Twenty-three catch sites handled only `ApiException`** — but `fromJson` throws `TypeError`, which slips past all of them, so a response this build did not expect left the sign-in button spinning for ever with nothing on screen; they now absorb anything via `ApiException.from`, which also strips the internal text. **A refused refresh cleared the tokens and nothing else**, so `isSignedIn` stayed true, the router kept the learner on a screen where every request 401'd, and nothing explained why; the API layer now raises a signal the session controller acts on — signed out, sent to the form, told their session ended. **An error thrown inside `build`** showed Flutter's red or grey screen; release builds get a plain bilingual panel instead. Retry is offered only where pressing it could change the answer — `SESSION_STARTING` is a 409 whose whole meaning is "ask again", and it was the one refusal telling the learner to retry with no way to. Also found by asking what the API would refuse: the sign-in form required six characters where `RegisterRequest` requires eight, so a seven-character password passed the form, was refused by the server, and came back naming no field. 72 new tests, including one that fails if a future error code is added to the backend and not to the app. 352 Flutter tests. |
